...
| 向量圖解 | `figs/`（子資料夾） | 程式繪製的 SVG／PNG 與生成腳本，見 [§5.1](#5-1-figs) | Cowork |
...
PNG 截圖是**使用者提供的既有影像**；向量圖解是 **Cowork 以程式繪製、可重跑的產物**，兩者來源不同，故分開存放。

```
raw/solutions/RC-YYYY-N/figs/
    RC-YYYY-N-fig-M-[內容碼].svg    ← 給 HTML wiki 與 WeasyPrint
    RC-YYYY-N-fig-M-[內容碼].png    ← 2× 點陣版，給 pptx／影片管線
    gen_RC-YYYY-N.py                ← 生成腳本，必須可重跑
```

| 規則 | 說明 |
|------|------|
| 序號 `M` | 依圖在 .md 中出現順序，從 `1` 起 |
| 內容碼 | 描述圖的內容，小寫連字號，如 `section`、`mn-c`、`balanced`、`pm` |
| SVG／PNG 成對 | 同名兩份，PNG 為 2× 解析度 |
| 生成腳本 | 一題一支 `gen_RC-YYYY-N.py`，所有幾何由座標與解題算出的數值決定，不得手繪或 AI 生圖 |
| .md 引用路徑 | `figs/RC-YYYY-N-fig-M-[內容碼].svg`（相對於解析主檔） |
| 圖說 | 每張圖下方以 `*圖 M　…*` 起始，說明這張圖能攔下哪個陷阱 |

> **與考卷原圖並存：** 向量重繪不取代 `RC-YYYY-N-fig-N.png`。考卷原始附圖有存證價值，兩者在 §1 並列，圖說分別標為「考卷原圖」與「向量重繪」（編號 `圖 1a`／`圖 1b`）。

> **數值一致性：** 生成腳本會重算解題數值。若腳本算出的值與 .md 既有數值不符（多為四捨五入累積誤差），**以腳本為準**並同步修正 .md、`wiki/problems/` 摘要頁與 `study/problems-view/` 渲染頁三處。
