...
看到「耐震設計」或「韌性」
    │
    ├─ 有「梁」「特殊矩形框架梁」
    │       → 密箍區長度（ACI 318 §18.6.4.1）：lo = 2h（單一條件，自柱面量起）
    │         ※ 梁只有 2h，沒有 ln/6、ln/4 或 450mm 這些條件（那是柱 §18.7.5.1）
    ...
    │           0.09·s·hc·f'c/fyh
    │         )
    │         密箍區間距：s ≤ min(最小邊長/4, 6db_主筋, so)；so = 10+(35-hx)/3 ≤ 15cm
    ...
    │                       T = 1.25fy·As（用 Mpr）
    │         接頭強度：φVjh ≤ φγ√f'c·Aj
    │           γ（kgf/cm² 制）= 5.3（內接頭，四面有梁圍束）
    │                            4.0（T 形，三面或對面兩面圍束）
    │                            3.2（外接頭／角接頭，其他）
    │           ＝ MPa 制 1.7／1.25／1.0 ＝ psi 制 20／15／12（同一組值）
    │         Aj = 有效接頭面積（柱寬 × 有效深度）
    │         Concept: [[SEISMIC-DESIGN]]
    │
    ├─ 有「強柱弱梁」「ΣMnc」「ΣMnb」
...
- **lo 的公式**：梁只有單一條件 `2h`（§18.6.4.1）；柱才是三式取大 `max(h, lu/6, 450mm)`（§18.7.5.1）。
  把柱的 `ln/6`、`450mm` 套到梁上是常見錯誤（多半不影響答案，因 2h 通常控制，但會被扣過程分）
- **接頭 γ 係數**：內/T形/其他各不同，且**數字隨單位制改變**（kgf/cm² 5.3/4.0/3.2）
